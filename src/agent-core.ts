...
// Vercel AI SDK primitives for both the streaming and batch code paths,
// plus the shared tool definitions (generateDiagram / modifyDiagram).
import {
  generateText,
  streamText,
  stepCountIs,
...
// Single source of truth for how the agent should behave. Changing this
// affects the live worker and every eval run identically.
...
// Shared inputs for both streamAgent and runAgent.
interface AgentArgs {
  model: LanguageModel;
...
// Loose shape so this works against both generateText's real step objects
// and any lightweight fixture the tests want to pass in.
interface StepLike {
  ...
  for (const step of steps) {
    for (const toolResult of step.toolResults ?? []) {
      if (toolResult.toolName === "generateDiagram") {
        // Full replace: a fresh diagram overwrites whatever was on the canvas.
        const output = toolResult.output as any;
        if (Array.isArray(output?.elements)) {
          canvas = [...output.elements];
        }
      } else if (toolResult.toolName === "modifyDiagram") {
        // Partial patch: merge updates into the matching element by id,
        // leaving everything else on the canvas untouched.
        const output = toolResult.output as any;
