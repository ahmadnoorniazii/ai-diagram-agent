...
// The production system prompt: role framing, hard rules for valid Excalidraw
// elements, a mechanical layout grid (models are bad at picking coordinates),
// per-diagram-type patterns, explicit negative prompts, and worked examples
// for both create and modify flows. This supersedes the simpler prompt in
// system-prompt.ts, which is now only used by the eval harness as a baseline.
...
// Shared input shape for both the streaming (worker) and batch (eval) entry
// points below, so they can't drift on what a call needs to provide.
interface AgentArgs {
  model: LanguageModel;
...
// Appends a TOON-encoded snapshot of the current canvas to the base system
// prompt so the model always knows what elements (and ids) already exist.
