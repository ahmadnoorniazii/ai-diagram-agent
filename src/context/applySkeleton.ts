...
//   stamps, anything visual. The scorers don't need it and the code
//   should stay readable.

// Loose type aliases rather than full Excalidraw types — this file only
// cares about the handful of fields it reads/writes, not the complete
// element shape.
...
export function applySkeleton(skeletons: SkeletonElement[]): RuntimeElement[] {
  ...
  // Route each element to the handler for its type. Shapes and arrows get
  // special handling to unpack labels/bindings into runtime fields; anything
  // else (standalone text) passes through unchanged.
  for (const el of cleaned) {
    ...
    if (type === "arrow" || type === "line") {
      ...
      // same pattern as containers. Most diagrams don't use
      // them, but this example calls them out for "yes/no" decisions.
      if (label && typeof label === "object") {
        const labelObj = label as Record<string, unknown>;
        const text = labelObj.text;
        if (typeof text === "string" && text.length > 0) {
