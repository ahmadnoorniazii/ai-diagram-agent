...
//   sees the new arrows.
//
// The interesting code is in `applySkeleton.ts` and the
// noOverlaps scorer; this is just plumbing around an Excalidraw helper
// limitation.

// Minimal shapes for the three element representations this helper juggles:
// the model's raw skeleton input, the converted runtime Excalidraw element,
// and an element already sitting on the canvas.
...
export function applyCrossCallBindings<T extends ExistingLike>(
  ...
  // Walk each connector skeleton and, for any endpoint that already resolved
  // successfully within this batch (skeleton helper set it), leave it alone;
  // only patch endpoints pointing at an id from a prior call that the
  // skeleton helper couldn't see and therefore dropped.
