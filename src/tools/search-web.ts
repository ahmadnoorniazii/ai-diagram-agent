...
// a demonstration of "tool execute can call an external API."
//
...
//
// Three reasons web search lives here vs context vs RAG:
// - Context: facts you ALWAYS want the model to see (system prompt, canvas
//   state). Cheap, but bloats every request.
// - Tools (this approach): facts you fetch ON DEMAND when the model decides it
//   needs them. The model controls the trigger.
// - RAG: facts retrieved from YOUR OWN corpus via embeddings.
//   Same on demand pattern, different data source.
// Web search is the simplest version of "model decides to fetch external data."

// Only the fields we actually forward to the model; Tavily's response has
// more, but there's no reason to hand the model extra tokens it won't use.
...
Example: searchWeb({ query: "how Cloudflare Workers handle incoming requests", maxResults: 5 })`,
    ...
    execute: async ({ query, maxResults }) => {
      // Missing key is a config problem, not a crash — hand it back as a
      // normal tool result so the model can tell the user instead of the
