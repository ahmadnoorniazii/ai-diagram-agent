...
// Factory rather than a static tool export so callers (Worker per-request
// env, or the eval harness) can inject their own Upstash credentials.
...
Example: searchKnowledge({ query: "OAuth 2.0 authorization code flow with PKCE" })`,
    ...
    execute: async ({ query }) => {
      try {
        // `data: query` triggers Upstash's server-side embedding of the raw
        // query text (same hosted embedding model used at index time), so no
        // client-side embedding call is needed here.
        ...
        // Pull the original document text back out of metadata.content
        // (stashed there during embed) since the vector `data` field itself
        // isn't returned by query().
        return {
        ...
        // Swallow errors into the tool result instead of throwing, so a
        // misconfigured/unreachable index degrades the agent's context
