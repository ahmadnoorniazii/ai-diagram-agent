...
async function main() {
  // Uses process.env directly since this runs as a plain Node script (via
  // `npm run embed`), not inside the Worker request path.
  const index = getIndex({
    UPSTASH_VECTOR_REST_URL: process.env.UPSTASH_VECTOR_REST_URL,
    UPSTASH_VECTOR_REST_TOKEN: process.env.UPSTASH_VECTOR_REST_TOKEN,
  });

  // Wipe the index first so stale/removed corpus files never linger as
  // orphaned vectors — the corpus directory is the single source of truth.
  ...
  // Upsert one record per corpus file. Failures are logged and counted but
  // don't abort the run, so one bad file doesn't block the rest of the corpus.
  let ok = 0;
  let failed = 0;
  ...
  // Non-zero exit on any failure so CI/manual runs notice a partial embed.
