...
// Both weaknesses will show up in evals and get improved later.

export const tools = {
  // Asks the model to emit the whole diagram (every shape, label, and
  // connector) as one array in a single tool call.
  generateDiagram: tool({
    ...
    // Mirrors the ExcalidrawElement union from schemas.ts, but flattened
    // into one object (all shape-specific fields optional) since a single
    // Zod schema has to describe every element type the array can contain.
  ...
  // Patches a single existing element by id rather than regenerating the
  // whole diagram; the caller must already know the target element's id.
  modifyDiagram: tool({
    description:
