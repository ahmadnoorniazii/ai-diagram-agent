...
//   The system prompt told the model this in prose. The schema work below
//   makes it real by adding `containerId` to the schema and writing the field
//   description that explains when to use it.
//
// Why this scorer is such a visceral demo:
//   The earlier baseline diagrams have boxes with no visible
//   labels. They look like empty rectangles next to floating text. After
//   the schema work, the boxes are labeled. The eval number
//   matches what's visible on the canvas: bound labels means readable
//   diagrams.
//
// Why it's added alongside the schema field:
//   The earlier schema doesn't have `containerId` in it. Without the field,
//   the model can't actually do the right thing even if it wanted to. The
//   scorer would always read 0 and never move. We add the scorer alongside
//   the schema field, so the metric goes from "broken" to
//   "fixed" in one shot.
//
...
// Only these three element types can act as a label container.
