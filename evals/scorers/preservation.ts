...
export const preservationScorer: EvalScorer<
  ...
  GoldenTestCase
> = ({ output, expected }) => {
  // Only modify cases declare preservedIds; create cases skip this scorer.
  ...
  // Index of ids the agent's output actually contains, for O(1) lookups below.
  ...
  // For each id we expect to survive, check presence first, then that its
  // type wasn't silently swapped out from under it.
