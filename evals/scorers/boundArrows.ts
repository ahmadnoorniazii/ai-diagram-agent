...
//   Before the schema description work below, the agent constantly produced
//   arrows with no bindings, or with bindings that pointed at ids it had
//   hallucinated. Visually those arrows render as floating lines next to the
//   diagram, completely detached from the boxes they're supposed to connect.
//   Schema didn't catch it (the arrow had all required fields). Structure
//   didn't catch it (the count was right). LabelKeywords didn't catch it (the
//   words were elsewhere). The diagram looked broken to a human and passed
//   every existing scorer.
//
// Why it's added alongside the schema description work:
//   The system prompt tells the model to bind both ends of every
//   connecting arrow, but the schema doesn't enforce it. Without the
...
//   often enough that the metric isn't actionable. We add the scorer alongside
//   the fix, so the eval shows a clean before/after.
//
...
// Ratio of arrows with two live, resolvable bindings to total arrows.
