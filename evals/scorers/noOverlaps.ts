...
// Braintrust scorer type plus the shapes of the agent's output and the
// golden test case it's graded against.
import type { EvalScorer } from "braintrust";
import type { AgentOutput } from "./schema";
import type { GoldenTestCase } from "../buildMessages";
// Reuse the exact overlap detection the live agent uses (see App.tsx /
// agent-core.ts) so the eval and the runtime never disagree on what counts
// as an overlap.
import { findOverlaps, countOverlapEligiblePairs } from "../../src/context/overlaps";

// Braintrust scorer entry point: receives the agent's produced elements and
// returns a graded score plus metadata for the eval report.
export const noOverlapsScorer: EvalScorer<GoldenTestCase, AgentOutput, GoldenTestCase> = ({
  ...
  // Nothing to compare (0 or 1 eligible elements) — abstain rather than
  // report a misleading perfect score.
  if (totalPairs === 0) return null;

  const overlapping = findOverlaps(elements);
  // Graded score: fraction of eligible pairs that do NOT overlap.
