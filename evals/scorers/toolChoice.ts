...
// to the earlier modifyDiagram tool surface and stopped meaning much once
...
export const toolChoiceScorer: EvalScorer<GoldenTestCase, AgentOutput, GoldenTestCase> = ({
  ...
  // create/domain: a new diagram must actually get drawn.
  ...
  // modify: editing an existing diagram requires knowing its current ids
  // first, so queryCanvas must precede the mutating call.
  if (category === "modify") {
  ...
  // edge: no tool-choice expectation for this category, so this scorer
  // doesn't apply — Braintrust skips a null score rather than counting it.
  return null;
};
